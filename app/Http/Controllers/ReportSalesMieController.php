...
class ReportSalesMieController extends Controller
{
    /**
    ...
    public function export($type, Request $request)
    {
        ...
        // }
                // dd($filter);
        Excel::create('sales_mie_supremie_'.Carbon::now()->format('Ymdhis'), function($excel) use ($list,$filter) {

            // Set the spreadsheet title, creator, and description
            $excel->setTitle('Sale Mie Reports');
            $excel->setCreator('Permadi Wibisono')->setCompany('Supremie');
            $excel->setDescription('Sale Mie Reports');
            ...
            $excel->sheet('sheet1', function($sheet) use ($list,$filter,$options) {
                ...
                $sheet->loadView('reports.export.sales_mie_xls',array('list' => $list,'sub_title'=>$sub_title));
            });
